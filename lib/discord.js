...
function findClass(input) {
  ...
  return CLASSES.find((c) => c.toLowerCase() === trimmed) || null;
}
  ...
  if (!member) {
    if (!classOption) {
      return ephemeralMessage(
        `No guild member named "${displayName}" was found in Capital Records, so I can't record this yet. Run /growth again and fill in the "class" option too (type your class name), and I'll add you to the roster automatically.\n\nValid classes: ${CLASSES.join(', ')}`
      );
    }
    const className = findClass(classOption.value);
    if (!className) {
      return ephemeralMessage(
        `"${classOption.value}" isn't a valid class. Please run /growth again with one of these exact names for "class":\n${CLASSES.join(', ')}`
      );
    }
...
const GROWTH_COMMAND = {
  ...
  options: [
    ...
    {
      name: 'class',
      description: 'Your class, typed exactly (only needed the first time, to add you to the roster)',
      type: 3, // STRING
      required: false,
    },
  ],
