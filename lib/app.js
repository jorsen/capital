...
async function fetchLootRecords(sessionIds) {
  ...
  const { rows } = await sql`
    SELECT id, session_id, recipient_id, recipient_name, item, quantity, notes, via_raffle, excluded_from_raffle, created_at
    FROM loot_records
...
app.put('/api/loot/:id/records/:recordId', async (req, res) => {
  ...
  const { recipientId, item, quantity, notes, viaRaffle, excludedFromRaffle } = req.body || {};
  ...
  const { rows } = await sql`
    ...
    SET recipient_id = ${nextRecipientId}, recipient_name = ${nextRecipientName},
        ...
        excluded_from_raffle = ${nextExcludedFromRaffle}
