...
function serializeCrusadeFee(row) {
  return { id: row.id, crusadeId: row.crusade_id, name: row.name, percent: Number(row.percent) };
}
...
app.get('/api/crusades', async (req, res) => {
  const { rows } = await sql`
    SELECT c.*, (SELECT COUNT(*)::int FROM crusade_participants p WHERE p.crusade_id = c.id) AS participant_count
    FROM crusades c
    ORDER BY c.created_at DESC
  `;
  res.json(rows.map((r) => ({ ...serializeCrusade(r), participantCount: r.participant_count })));
});
...
app.post('/api/crusades', async (req, res) => {
  ...
  res.status(201).json({ ...serializeCrusade(rows[0]), participantCount: 0 });
});
...
app.post('/api/crusades/:id/fees', async (req, res) => {
  ...
  const id = crypto.randomUUID();
  await sql`INSERT INTO crusade_fees (id, crusade_id, name, percent) VALUES (${id}, ${req.params.id}, ${name.trim()}, ${finalPercent})`;
  ...
  await logActivity(req, {
    ...
    after: { name: name.trim(), percent: finalPercent },
  });
