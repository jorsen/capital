...
async function getLastCrusadeBidders(crusade) {
  if (!crusade.event_date) return [];
  const { rows: prevRows } = await sql`
    SELECT id FROM crusades
  ...
  const { rows: bidderRows } = await sql`
    ...
    WHERE crusade_id = ${prevRows[0].id} AND gold_bid > 0
    ORDER BY name ASC
  `;
  return bidderRows.map((r) => ({ name: r.name, guildName: r.guild_name, goldBid: Number(r.gold_bid) }));
}
...
app.get('/api/crusades/:id', async (req, res) => {
  ...
  const lastCrusadeBidders = await getLastCrusadeBidders(crusade);
  res.json({
