...
app.get('/api/crusades/:id', async (req, res) => {
  ...
  const { rows: participantRows } = await sql`
    SELECT * FROM crusade_participants WHERE crusade_id = ${req.params.id} ORDER BY party_number ASC, created_at ASC
  `;
  res.json({ ...serializeCrusade(crusade), participants: participantRows.map(serializeCrusadeParticipant) });
});
...
app.post('/api/crusades/:id/participants', async (req, res) => {
  ...
  const { name, guildName, position, goldBid, partyNumber, attended } = req.body || {};
  if (!name || !name.trim()) return res.status(400).json({ error: 'name is required' });
  ...
  const id = crypto.randomUUID();
  await sql`
    INSERT INTO crusade_participants (id, crusade_id, party_number, name, guild_name, position, gold_bid, attended)
    VALUES (${id}, ${req.params.id}, ${finalPartyNumber}, ${name.trim()}, ${guildName || null}, ${position || null}, ${finalGoldBid}, ${attended !== false})
  `;
...
app.put('/api/crusades/:id/participants/:pid', async (req, res) => {
  ...
  const { name, guildName, position, goldBid, partyNumber, attended, paid } = req.body || {};
  let nextName = existing.name;
  ...
  const { rows } = await sql`
    UPDATE crusade_participants SET
      ...
      gold_bid = ${nextGoldBid}, party_number = ${nextPartyNumber}, attended = ${nextAttended}, paid = ${nextPaid}
