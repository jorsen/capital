...
app.all('/api/discord/poll-boss-kills', async (req, res) => {
  ...
  try {
    ...
    // mention sitting further back in a since-idle cursor window.
    ...
    // Keep the most recent matching message's timestamp per boss, in case
    // more than one mention shows up within a single poll window.
...
// next spawn falls within the next 2 minutes. Meant to be hit by an external
// scheduler roughly once a minute — daily and Vercel Cron's coarser cadence
// are both too slow to reliably catch a 2-minute window.
const NOTIFY_LEAD_MS = 2 * 60 * 1000;

// Matches the buffer in boss-timer-view.js's nextSpawnMs() — respawn
// intervals are estimated from chat history, so this pads past the raw
// calculation to keep the notification from firing before a boss is
// actually confirmed up.
...
app.all('/api/discord/notify-spawns', async (req, res) => {
  ...
  try {
    ...
    const notified = [];
    for (const boss of bosses) {
      const nextSpawnMs = new Date(boss.last_killed_at).getTime() + boss.interval_minutes * 60000 + SPAWN_BUFFER_MS;
      ...
      const discordRes = await fetch(`https://discord.com/api/v10/channels/${channelId}/messages`, {
        ...
        body: JSON.stringify({ content: `@everyone ⏰ **${boss.name}** spawns in about 2 minutes!` }),
      });
      if (!discordRes.ok) {
