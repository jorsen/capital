...
app.all('/api/discord/poll-boss-kills', async (req, res) => {
  ...
  try {
    ...
    // Guards against ever logging (or Discord-confirming) the same message
    // twice -- the poll cursor below is the normal defense against
    // reprocessing, but this covers it too if that cursor save ever fails
    // to land, or two poll runs happen to overlap.
    const { rows: alreadyLoggedRows } = await sql`
      SELECT message_id FROM boss_kill_history WHERE message_id = ANY(${messages.map((m) => m.id)})
    `;
    const alreadyLoggedMessageIds = new Set(alreadyLoggedRows.map((r) => r.message_id));
    ...
    for (const msg of messages) {
      ...
      if (explicit && explicitBoss) {
        // Already logged (and confirmed) this exact message on a previous
        // run -- still lets it count toward setIfNewer so this boss's
        // timer reflects it, but never re-inserts history or re-posts the
        // Discord confirmation for it.
        if (alreadyLoggedMessageIds.has(msg.id)) {
          setIfNewer(explicitBoss.id, explicit.killedAt);
          continue;
        }
        setIfNewer(explicitBoss.id, explicit.killedAt);
        await sql`
          INSERT INTO boss_kill_history (id, boss_id, boss_name, killed_at, source, discord_author, message_id)
          VALUES (${crypto.randomUUID()}, ${explicitBoss.id}, ${explicitBoss.name}, ${explicit.killedAt}, 'discord', ${author}, ${msg.id})
        `;

        // No Discord confirmation posted from here on purpose -- an explicit
        // "Boss: X / Time of Death: Y" message is also exactly what the cave
        // attendance poller watches for on the same channel, and it already
        // posts its own "✅ Logged Successfully" reply (with matched
        // attendees) for this same message. Posting a second one here isn't
        // a race or a duplicate bug, it's two independent systems both
        // reacting to one message -- confirmed by boss_kill_history and
        // cave_sessions rows landing within the same second of each other
        // for the same kill. This branch only needs to update the respawn
        // timer; the human-facing confirmation is the other poller's job.
        continue;
      }

      if (alreadyLoggedMessageIds.has(msg.id)) continue;
      ...
      for (const boss of bosses) {
        if (content.includes(boss.name.toLowerCase())) {
          ...
          await sql`
            INSERT INTO boss_kill_history (id, boss_id, boss_name, killed_at, source, discord_author, message_id)
            VALUES (${crypto.randomUUID()}, ${boss.id}, ${boss.name}, ${msg.timestamp}, 'discord', ${author}, ${msg.id})
          `;
        }
      }
    }
...
app.all('/api/discord/notify-spawns', async (req, res) => {
  ...
  try {
    ...
    for (const boss of bosses) {
      ...
      // retriesLeft: 0 -- see the same note on the boss-kill confirmation
      // post above; a 429 retry here risks a duplicate "@everyone" ping.
      const discordRes = await discordFetch(
        `https://discord.com/api/v10/channels/${channelId}/messages`,
        {
          method: 'POST',
          headers: { Authorization: `Bot ${botToken}`, 'Content-Type': 'application/json' },
          // Text always says 5 even though notify_lead_minutes is set wider
          // (10) for reliability -- that margin is just insurance against a
          // poll landing late in the window, not something worth exposing in
          // the message itself.
          body: JSON.stringify({ content: `@everyone ⏰ **${boss.name}** spawns in about 5 minutes!` }),
        },
        0
      );
      if (!discordRes.ok) {
...
app.get('/api/item-categories', async (req, res) => {
  const { rows } = await sql`SELECT id, name, icon_url, hidden FROM item_categories ORDER BY created_at ASC`;
  res.json(rows.map((r) => ({ id: r.id, name: r.name, iconUrl: r.icon_url, hidden: r.hidden })));
});
...
app.put('/api/item-categories/:id', async (req, res) => {
  ...
  const { name, iconUrl, hidden } = req.body || {};
  let nextName = existing.name;
  if (name !== undefined) {
    if (!name.trim()) return res.status(400).json({ error: 'name cannot be empty' });
    nextName = name.trim();
  ...
  const nextHidden = hidden !== undefined ? !!hidden : existing.hidden;

  await sql`UPDATE item_categories SET name = ${nextName}, icon_url = ${nextIcon}, hidden = ${nextHidden} WHERE id = ${req.params.id}`;
  await logActivity(req, {
    action: 'update',
    entityType: 'item_category',
    entityId: req.params.id,
    description: `Updated item "${nextName}"`,
    before: { name: existing.name, iconUrl: existing.icon_url, hidden: existing.hidden },
    after: { name: nextName, iconUrl: nextIcon, hidden: nextHidden },
  });
  res.json({ id: req.params.id, name: nextName, iconUrl: nextIcon, hidden: nextHidden });
});
...
app.post('/api/loot/:id/submit-attendance', async (req, res) => {
  ...
  if (matchedBoss) {
    ...
    if (!channelId || !botToken) {
      ...
      try {
        // retriesLeft: 0 -- same reasoning as the other message-posting
        // calls above: a 429 retry resends the identical request, which can
        // double-post this if Discord's response was ambiguous.
        const discordRes = await discordFetch(
          `https://discord.com/api/v10/channels/${channelId}/messages`,
          {
            method: 'POST',
...
// Standing crow (🐦‍⬛) -> PHP conversion rate, e.g. 33 means 1 crow = ₱33.
app.get('/api/world-dungeon-settings', async (req, res) => {
  const { rows } = await sql`SELECT php_per_crow FROM world_dungeon_settings WHERE id = 1`;
  res.json({ phpPerCrow: Number(rows[0]?.php_per_crow || 0) });
});

app.put('/api/world-dungeon-settings', async (req, res) => {
  const { phpPerCrow } = req.body || {};
  const value = Number(phpPerCrow);
  if (Number.isNaN(value) || value < 0) return res.status(400).json({ error: 'phpPerCrow must be zero or a positive number' });

  await sql`
    INSERT INTO world_dungeon_settings (id, php_per_crow, updated_at) VALUES (1, ${value}, now())
    ON CONFLICT (id) DO UPDATE SET php_per_crow = ${value}, updated_at = now()
  `;
  await logActivity(req, {
    action: 'update',
    entityType: 'world_dungeon_settings',
    entityId: '1',
    description: `Set World Dungeon crow → PHP rate to ₱${value} per crow`,
    after: { phpPerCrow: value },
  });
  res.json({ phpPerCrow: value });
});

// Standing per-member "has GCash on file" flag -- not month-scoped, same
// shape as the multiplier table above.
app.get('/api/world-dungeon-gcash', async (req, res) => {
  const { rows } = await sql`SELECT member_id, has_gcash FROM world_dungeon_gcash`;
  res.json(rows.map((r) => ({ memberId: r.member_id, hasGcash: r.has_gcash })));
});

app.put('/api/world-dungeon-gcash/:memberId', async (req, res) => {
  const { hasGcash } = req.body || {};
  const { rows: memberRows } = await sql`SELECT id, name FROM members WHERE id = ${req.params.memberId}`;
  if (!memberRows[0]) return res.status(400).json({ error: 'member does not exist' });

  const value = !!hasGcash;
  await sql`
    INSERT INTO world_dungeon_gcash (member_id, has_gcash, updated_at) VALUES (${req.params.memberId}, ${value}, now())
    ON CONFLICT (member_id) DO UPDATE SET has_gcash = ${value}, updated_at = now()
  `;
  await logActivity(req, {
    action: 'update',
    entityType: 'world_dungeon_gcash',
    entityId: req.params.memberId,
    description: `Marked "${memberRows[0].name}" as ${value ? 'having' : 'not having'} GCash on file`,
    after: { hasGcash: value },
  });
  res.json({ memberId: req.params.memberId, hasGcash: value });
});
...
app.get('/api/item-send-status', async (req, res) => {
  const { itemName } = req.query;
  if (itemName) {
    const { rows } = await sql`SELECT member_id FROM item_send_status WHERE item_name = ${itemName}`;
    return res.json(rows.map((r) => r.member_id));
  }
  // No itemName -- returns every row across all items, so the Top 20
  // checklist can render one Sent column per item from a single request
  // instead of one round trip per item.
  const { rows } = await sql`SELECT item_name, member_id FROM item_send_status`;
  res.json(rows.map((r) => ({ itemName: r.item_name, memberId: r.member_id })));
});

app.post('/api/item-send-status', async (req, res) => {
  const { itemName, memberId } = req.body || {};
  if (!itemName || !memberId) return res.status(400).json({ error: 'itemName and memberId are required' });
  const { rows: memberRows } = await sql`SELECT id, name FROM members WHERE id = ${memberId}`;
  if (!memberRows[0]) return res.status(400).json({ error: 'member does not exist' });

  const id = crypto.randomUUID();
  await sql`
    INSERT INTO item_send_status (id, item_name, member_id)
    VALUES (${id}, ${itemName}, ${memberId})
    ON CONFLICT (item_name, member_id) DO NOTHING
  `;
  await logActivity(req, {
    action: 'update',
    entityType: 'item_send_status',
    entityId: memberId,
    description: `Marked "${memberRows[0].name}" as sent ${itemName}`,
    before: { sent: false },
    after: { sent: true },
  });
  res.status(201).json({ itemName, memberId });
});

app.delete('/api/item-send-status', async (req, res) => {
  const { itemName, memberId } = req.query;
  if (!itemName || !memberId) return res.status(400).json({ error: 'itemName and memberId are required' });
  await sql`DELETE FROM item_send_status WHERE item_name = ${itemName} AND member_id = ${memberId}`;
  await logActivity(req, { action: 'update', entityType: 'item_send_status', entityId: memberId, description: `Unmarked a member as sent ${itemName}`, before: { sent: true }, after: { sent: false } });
  res.status(204).end();
});
...
async function processCaveAttendanceMessages(messages, { channelId, botToken, year }) {
  ...
  for (const msg of messages) {
    ...
    if (logged) {
      // Discord itself silently touches a message shortly after it's
      // posted with an attachment (generating embeds/proxy URLs), which can
      // set edited_timestamp on a message nobody actually edited -- that
      // touch-up lands within seconds of the original post, while a real
      // human edit (fixing a typo, adding a forgotten name) happens
      // noticeably later. Requiring a real gap between posted and "edited"
      // filters out the phantom one, which was re-triggering (and
      // re-posting) a confirmation for every attendance message with a
      // screenshot attached.
      const EDIT_GRACE_MS = 2 * 60 * 1000;
      const isMeaningfulEdit =
        !!msg.edited_timestamp && new Date(msg.edited_timestamp).getTime() - new Date(msg.timestamp).getTime() > EDIT_GRACE_MS;
      const editIsNewer = isMeaningfulEdit && (!logged.edited_at || new Date(msg.edited_timestamp) > new Date(logged.edited_at));
      if (!editIsNewer) {
        results.push({ messageId: msg.id, skipped: true, reason: 'already logged' });
        continue;
      }
      // Claims this specific edit transition atomically, the same reason
      // the brand-new-message branch below claims before doing anything
      // else: logged.edited_at above is a stale snapshot, so two
      // overlapping poll runs can both read the same old edited_at and
      // both decide "this is a newer edit" for the same message. The WHERE
      // clause re-checks edited_at against the row's value at UPDATE time,
      // so Postgres's row lock serializes the two attempts -- only the
      // first to commit affects a row, and the loser skips instead of
      // re-posting a confirmation for an edit someone else already handled.
      const { rows: editClaimRows } = await sql`
        UPDATE cave_attendance_logged_messages
        SET edited_at = ${msg.edited_timestamp}
        WHERE message_id = ${msg.id}
          AND (edited_at IS NULL OR edited_at < ${msg.edited_timestamp})
        RETURNING message_id
      `;
      if (!editClaimRows.length) {
        results.push({ messageId: msg.id, skipped: true, reason: 'already logged' });
        continue;
      }
      editedSinceLogged = true;
    } else {
      // Claims this message id atomically before doing anything else with
      // it -- loggedByMessageId above is just a snapshot taken once at the
      // top of this call, so two overlapping poll runs (the auto-poll
      // landing on top of another one still finishing, or a manual sync
      // racing the auto-poll) could otherwise both see "not logged yet" for
      // the same brand-new message and both create a session and post a
      // confirmation for it. A conflict here means some other run claimed
      // it first, so this one backs off instead of duplicating that work.
      // session_id starts NULL and gets filled in by the upsert below once
      // the real session exists (see the orphan sweep further down for
      // recovering a claim that crashed before reaching that point).
      const { rows: claimRows } = await sql`
        INSERT INTO cave_attendance_logged_messages (message_id, edited_at, session_id)
        VALUES (${msg.id}, ${msg.edited_timestamp || null}, NULL)
    ...
    // Whether this iteration just inserted a fresh claim row above -- if
    // this message turns out to have no boss name or no recognizable
    // attendees, that claim needs cleaning up rather than left behind as a
    // permanent session_id-less row (which would otherwise look like a
    // crashed-mid-processing message to the orphan-recovery sweep below).
    const freshlyClaimed = !logged;
    ...
    if (!bossName) {
      if (freshlyClaimed) await sql`DELETE FROM cave_attendance_logged_messages WHERE message_id = ${msg.id} AND session_id IS NULL`;
    ...
    if (isRecent) {
      try {
        // retriesLeft: 0 -- a 429 retry resends the identical request, and
        // if Discord's own response was just ambiguous under load, that
        // retry is what actually double-posts this confirmation (the
        // attendance itself is already saved above regardless of whether
        // this post lands at all, let alone twice).
        const confirmRes = await discordFetch(
          `https://discord.com/api/v10/channels/${channelId}/messages`,
          {
            method: 'POST',
            headers: { Authorization: `Bot ${botToken}`, 'Content-Type': 'application/json' },
