...
app.post('/api/cave-attendance-sync', async (req, res) => {
  ...
  if (!channel) return res.status(404).json({ error: 'no channel named "cave-attendance" found in this Discord server' });
  ...
  for (const msg of messages) {
    ...
    let discordPosted = false;
    try {
      const confirmRes = await fetch(`https://discord.com/api/v10/channels/${channel.id}/messages`, {
        method: 'POST',
        headers: { Authorization: `Bot ${botToken}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({
          content: [
  ...
  res.json({ channelId: channel.id, scanned: messages.length, results });
});
