...
app.put('/api/loot/:id', async (req, res) => {
  ...
  await logActivity(req, {
    ...
    before: { date: existing.date, run: existing.run, notes: existing.notes, absentees: existing.absentees || [] },
    after: { date: rows[0].date, run: rows[0].run, notes: rows[0].notes, absentees: rows[0].absentees || [] },
  });
  res.json(serializeLootSession(rows[0], recordsMap.get(rows[0].id)));
});
...
app.put('/api/caves/:id', async (req, res) => {
  ...
  await logActivity(req, {
    ...
    before: { date: existing.date, run: existing.run, notes: existing.notes, attendees: existing.attendees || [] },
    after: { date: rows[0].date, run: rows[0].run, notes: rows[0].notes, attendees: rows[0].attendees || [] },
  });
  res.json(serializeCaveSession(rows[0], recordsMap.get(rows[0].id)));
});
