...
// Pulls candidate attendee names out of a message once the boss-name and
// date substrings (if found) are stripped out — handles numbered/bulleted
// lists, plain line-per-name, and comma-separated names on one line.
function extractNameTokens(content, stripSubstrings) {
  let text = content || '';
  for (const s of stripSubstrings.filter(Boolean)) {
    text = text.replace(new RegExp(escapeRegex(s), 'gi'), '');
  }
  const tokens = [];
  text.split(/\r?\n/).forEach((rawLine) => {
...
// Finds an explicit date (M/D, M/D/YYYY, or YYYY-MM-DD) written in the
// message — falls back to the message's own post date (in PH time) when
// nothing is found, since attendance is often typed up after the fact.
...
// Scans #cave-attendance for a given month, matches each message's attendee
// list against the roster (name/alias, fuzzy), and logs attendance onto the
// matching cave date + boss (creating the boss log if it doesn't exist yet).
// Posts a confirmation back into the channel for every session it touches so
// the match is visible for a human to sanity-check, rather than only living
// in the app.
app.post('/api/cave-attendance-sync', async (req, res) => {
  ...
  // Prefer an explicit channel id (env var, same pattern as
  // DISCORD_BOSS_CHANNEL_ID) since the by-name lookup only sees channels the
  // bot's own channel-list call returns — a channel it can't see there but
  // can still read/post in (e.g. permission quirks) still works by id.
  let channelId = process.env.DISCORD_CAVE_ATTENDANCE_CHANNEL_ID;
  if (!channelId) {
    const channelsRes = await fetch(`https://discord.com/api/v10/guilds/${guildId}/channels`, {
      headers: { Authorization: `Bot ${botToken}` },
    });
    if (!channelsRes.ok) {
      return res.status(502).json({ error: 'failed to list Discord channels', details: await channelsRes.text() });
    }
    const channels = await channelsRes.json();
    const channel = channels.find((c) => (c.name || '').toLowerCase() === 'cave-attendance');
    if (!channel) {
      return res.status(404).json({
        error: 'no channel named "cave-attendance" found in this Discord server — set DISCORD_CAVE_ATTENDANCE_CHANNEL_ID to its channel ID instead',
      });
    }
  ...
  for (const msg of messages) {
    ...
    const dateMatch = extractDateFromText(msg.content, year);
    const date = dateMatch ? dateMatch.date : fallbackDate;

    const tokens = extractNameTokens(msg.content, [bossName, dateMatch?.match]);
      ...
      const confirmRes = await fetch(`https://discord.com/api/v10/channels/${channelId}/messages`, {
        ...
        body: JSON.stringify({
          content: [
            ...
            `Attendees: ${matchedNames.join(', ')}`,
            unmatched.length ? `⚠️ Not recognized: ${unmatched.join(', ')}` : null,
          ].filter(Boolean).join('\n'),
