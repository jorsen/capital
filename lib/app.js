...
// that other crusade counts, not just its single highest-numbered one,
// and a source team's own win/lose doesn't matter, only that gold was bid
// there; a Defense-stance source team is excluded since it never had
// bidding to begin with. Returns both which crusade that was (for
// display) and the pooled bidder list.
async function getLastTeamBidders(crusade) {
  ...
  const { rows: sourceTeams } = await sql`
    SELECT team_number, stance FROM crusade_teams WHERE crusade_id = ${sourceCrusadeId}
  `;
