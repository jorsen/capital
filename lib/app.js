...
// Regular Discord message content has no color markdown at all -- the only
// way to get actual colored text is a fenced ```ansi code block using raw
// ANSI escape codes, which Discord's client (desktop/web) specifically
// renders with color instead of showing literally. Cycles through the
// handful of foreground colors Discord's ANSI support recognizes, bolded,
// for a rainbow effect; spaces are left uncolored (a space colored red vs.
// blue looks identical, so there's no point spending a code on one).
const RAINBOW_ANSI_FG_CODES = [31, 33, 32, 36, 34, 35]; // red, yellow, green, cyan, blue, pink
function rainbowAnsiBlock(text) {
  const colored = text
    .split('')
    .map((ch, i) => (ch === ' ' ? ch : `[1;${RAINBOW_ANSI_FG_CODES[i % RAINBOW_ANSI_FG_CODES.length]}m${ch}[0m`))
    .join('');
  return '```ansi\n' + colored + '\n```';
}
        ...
        const confirmRes = await discordFetch(`https://discord.com/api/v10/channels/${channelId}/messages`, {
          ...
          body: JSON.stringify({
            content: [
              ...
              rainbowAnsiBlock('THANK YOU FOR YOUR HARDWORK!'),
            ].filter(Boolean).join('\n'),
