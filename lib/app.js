...
app.put('/api/queue/:slot', async (req, res) => {
  ...
  const { rows: existingRows } = await sql`SELECT done FROM queue_slots WHERE slot = ${slot}`;
  const existingDone = existingRows[0]?.done || [];
  const nextDone = existingDone.filter((n) => cleaned.includes(n));
  await sql`
    INSERT INTO queue_slots (slot, names, done) VALUES (${slot}, ${JSON.stringify(cleaned)}, ${JSON.stringify(nextDone)})
    ON CONFLICT (slot) DO UPDATE SET names = ${JSON.stringify(cleaned)}, done = ${JSON.stringify(nextDone)}
  `;
  res.json({ slot, names: cleaned, done: nextDone });
});

app.put('/api/queue/:slot/done', async (req, res) => {
  const slot = req.params.slot;
  if (!SLOTS.includes(slot)) return res.status(400).json({ error: 'invalid slot' });

  const { done } = req.body || {};
  if (!Array.isArray(done) || !done.every((n) => typeof n === 'string')) {
    return res.status(400).json({ error: 'done must be an array of strings' });
  }

  const cleaned = done.map((n) => n.trim()).filter(Boolean);
  await sql`
    INSERT INTO queue_slots (slot, done) VALUES (${slot}, ${JSON.stringify(cleaned)})
    ON CONFLICT (slot) DO UPDATE SET done = ${JSON.stringify(cleaned)}
  `;
  res.json({ slot, done: cleaned });
});
