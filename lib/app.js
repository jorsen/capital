...
app.post('/api/loot/:id/raffle-draws', async (req, res) => {
  const { rows: sessionRows } = await sql`SELECT * FROM loot_sessions WHERE id = ${req.params.id}`;
  const session = sessionRows[0];
  if (!session) return res.status(404).json({ error: 'session not found' });

  const { item, winnerId } = req.body || {};
  ...
  const { rows: memberRows } = await sql`SELECT id, name FROM members WHERE id = ${winnerId}`;
  const winner = memberRows[0];
  if (!winner) return res.status(400).json({ error: 'winner does not exist' });
  ...
  const nextDraws = [...(session.raffle_draws || []), draw];

  await sql`UPDATE loot_sessions SET raffle_draws = ${JSON.stringify(nextDraws)} WHERE id = ${req.params.id}`;
  res.status(201).json(draw);
});

app.delete('/api/loot/:id/raffle-draws/:drawId', async (req, res) => {
  const { rows: sessionRows } = await sql`SELECT * FROM loot_sessions WHERE id = ${req.params.id}`;
  const session = sessionRows[0];
  if (!session) return res.status(404).json({ error: 'session not found' });

  const draws = session.raffle_draws || [];
  const nextDraws = draws.filter((d) => d.id !== req.params.drawId);
  if (nextDraws.length === draws.length) return res.status(404).json({ error: 'draw not found' });
