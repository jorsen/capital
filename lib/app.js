...
app.all('/api/discord/poll-boss-kills', async (req, res) => {
  ...
  try {
    ...
    // param used, so reverse to process in chronological order.
    ...
    if (messages.length) {
      ...
      await sql`
        INSERT INTO boss_kill_poll_state (id, last_message_id, updated_at) VALUES (1, ${newestId}, now())
        ON CONFLICT (id) DO UPDATE SET last_message_id = ${newestId}, updated_at = now()
      `;
    }
...
app.post('/api/boss-timers/:id/kill', async (req, res) => {
  const { rows } = await sql`
    UPDATE boss_timers SET last_killed_at = now() WHERE id = ${req.params.id} AND type = 'interval'
    RETURNING *
  `;
