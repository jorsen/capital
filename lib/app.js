...
function serializeLootSession(row, records) {
  return {
    ...
    raffleWinnerId: row.raffle_winner_id,
    createdAt: row.created_at,
    records: records || [],
  };
}
...
app.put('/api/loot/:id', async (req, res) => {
  ...
  const { date, run, notes, absentees, raffleWinnerId } = req.body || {};
  ...
  if (raffleWinnerId !== undefined && raffleWinnerId !== null && typeof raffleWinnerId !== 'string') {
    return res.status(400).json({ error: 'raffleWinnerId must be a member id or null' });
  }
  ...
  const nextRaffleWinnerId = raffleWinnerId !== undefined ? raffleWinnerId : existing.raffle_winner_id;

  const { rows } = await sql`
    UPDATE loot_sessions
    SET date = ${nextDate}, run = ${nextRun}, notes = ${nextNotes}, absentees = ${JSON.stringify(nextAbsentees)},
        raffle_winner_id = ${nextRaffleWinnerId}
    WHERE id = ${req.params.id}
    RETURNING *
  `;
  const recordsMap = await fetchLootRecords([rows[0].id]);
