...
app.post('/api/crusades', async (req, res) => {
  const { name, warType, stance, area, leader, result, diamondReward, attendancePct, itemName, itemQuantity, eventDate, notes } = req.body || {};
  if (!name || !name.trim()) return res.status(400).json({ error: 'name is required' });
  ...
  const finalItemQuantity = Number(itemQuantity) || 0;
  if (finalItemQuantity < 0) return res.status(400).json({ error: 'itemQuantity must be a non-negative number' });

  const id = crypto.randomUUID();
  await sql`
    INSERT INTO crusades (id, name, war_type, stance, area, leader, result, diamond_reward, attendance_pct, item_name, item_quantity, event_date, notes)
    VALUES (
      ${id}, ${name.trim()}, ${warType || null}, ${stance || null}, ${area || null}, ${leader || null},
      ${result || 'pending'}, ${finalDiamondReward}, ${finalAttendancePct}, ${(itemName && itemName.trim()) || null}, ${finalItemQuantity},
      ${eventDate || null}, ${notes || null}
    )
  `;
...
app.put('/api/crusades/:id', async (req, res) => {
  ...
  let nextItemName = existing.item_name;
  let nextItemQuantity = existing.item_quantity;
  ...
  if (itemName !== undefined) nextItemName = (itemName && itemName.trim()) || null;
  if (itemQuantity !== undefined) {
    const n = Number(itemQuantity);
    if (Number.isNaN(n) || n < 0) return res.status(400).json({ error: 'itemQuantity must be a non-negative number' });
    nextItemQuantity = n;
  }
  ...
  const { rows } = await sql`
    UPDATE crusades SET
      ...
      attendance_pct = ${nextAttendancePct}, item_name = ${nextItemName}, item_quantity = ${nextItemQuantity},
      event_date = ${nextEventDate}, notes = ${nextNotes}
    WHERE id = ${req.params.id}
    RETURNING *
  `;
  await logActivity(req, {
