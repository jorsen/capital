...
app.all('/api/discord/poll-boss-kills', async (req, res) => {
  ...
  try {
    ...
    const { rows: members } = await sql`SELECT id, name, alias FROM members`;
    ...
    for (const msg of messages) {
      ...
      if (explicit && explicitBoss) {
        ...
        // Same "✅ Logged Successfully" confirmation style as Cave
        // Attendance's — this only confirms the kill time was recorded
        // against this boss's respawn timer, it doesn't create a cave
        // session or track loot attendance the way that one does.
        const tokens = extractNameTokens(msg.content, explicitBoss.name);
        const matchedNames = [];
        const unmatched = [];
        for (const token of tokens) {
          const match = matchMemberForToken(token, members);
          if (match) matchedNames.push(match.member.name);
          else unmatched.push(token);
        }
        const localKilledAt = new Date(new Date(explicit.killedAt).getTime() + BOSS_CHAT_TIMEZONE_OFFSET_MINUTES * 60000);
        const killedDate = localKilledAt.toISOString().slice(0, 10);
        try {
          // retriesLeft: 0 -- a 429 retry resends the identical request, and
          // if Discord's own response was just ambiguous under load (rather
          // than a clean, guaranteed-not-sent rejection), that can double-
          // post this exact confirmation. A dropped confirmation here is
          // far less disruptive than a duplicate one, and the underlying
          // kill was already saved regardless of whether this post lands.
          const confirmRes = await discordFetch(
            `https://discord.com/api/v10/channels/${channelId}/messages`,
            {
              ...
              body: JSON.stringify({
                content: [
                  `✅ Logged Successfully — **${explicitBoss.name}** (${killedDate})`,
                  `Time of Death: ${formatPhTime(explicit.killedAt)}`,
                  matchedNames.length ? 'Attendees:' : null,
          ...
          if (!confirmRes.ok) console.error('boss-kill confirm post failed', explicitBoss.name, await confirmRes.text());
        } catch (err) {
          console.error('boss-kill confirm post failed', err);
        }
        continue;
      }
