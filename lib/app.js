...
async function fetchLootRecords(sessionIds) {
  ...
  const { rows } = await sql`
    SELECT id, session_id, recipient_id, recipient_name, item, quantity, notes, via_raffle, excluded_from_raffle, sent, created_at
    FROM loot_records
...
app.post('/api/loot/:id/records', async (req, res) => {
  ...
  const { recipientId, item, quantity, notes, viaRaffle } = req.body || {};
  ...
  const { rows } = await sql`
    INSERT INTO loot_records (id, session_id, recipient_id, recipient_name, item, quantity, notes, via_raffle)
    VALUES (${id}, ${req.params.id}, ${member ? member.id : null}, ${member ? member.name : ''}, ${trimmedItem}, ${qty}, ${trimmedNotes}, ${!!viaRaffle})
    RETURNING *
  `;
  const row = rows[0];
  res.status(201).json({
...
app.put('/api/loot/:id/records/:recordId', async (req, res) => {
  ...
  const { recipientId, item, quantity, notes, viaRaffle, excludedFromRaffle, sent } = req.body || {};
  ...
  const { rows } = await sql`
    ...
    SET recipient_id = ${nextRecipientId}, recipient_name = ${nextRecipientName},
        ...
        excluded_from_raffle = ${nextExcludedFromRaffle}, sent = ${nextSent}
