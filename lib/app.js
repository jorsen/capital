...
// Records one row per mutating action for the Activity Log page. Never
// throws — a logging failure should not take down the action it's
// describing, so errors are swallowed after a console warning.
async function logActivity(req, { action, entityType, entityId, description, user: explicitUser }) {
  const user = explicitUser || req.currentUser;
  if (!user) return;
  try {
    await sql`
      INSERT INTO activity_log (id, user_id, username, role, action, entity_type, entity_id, description)
      VALUES (${crypto.randomUUID()}, ${user.id}, ${user.username}, ${user.role}, ${action}, ${entityType}, ${entityId ?? null}, ${description})
    `;
  } catch (err) {
...
app.use(async (req, res, next) => {
  ...
  const needsAdmin =
    req.path.startsWith('/api/users') || req.path === '/api/activity-log' || ADMIN_ONLY_GET_PATHS.has(req.path);
...
app.delete('/api/users/:id', async (req, res) => {
  ...
  await logActivity(req, { action: 'delete', entityType: 'user', entityId: req.params.id, description: `Deleted user "${existing.username}"` });
  res.status(204).end();
});
...
app.get('/api/activity-log', async (req, res) => {
  ...
  const { rows } = await sql`
    SELECT id, username, role, action, entity_type, entity_id, description, created_at
    FROM activity_log
...
app.post('/api/classes', async (req, res) => {
  ...
  await logActivity(req, { action: 'create', entityType: 'class', entityId: id, description: `Added class "${trimmed}"` });
...
app.delete('/api/classes/:name', async (req, res) => {
  ...
  await logActivity(req, { action: 'delete', entityType: 'class', entityId: req.params.name, description: `Removed class "${req.params.name}"` });
...
app.post('/api/members', async (req, res) => {
  ...
  const { rows } = await sql`
    ...
    RETURNING *
  `;
  await logActivity(req, { action: 'create', entityType: 'member', entityId: rows[0].id, description: `Added member "${rows[0].name}"` });
...
app.put('/api/members/:id', async (req, res) => {
  ...
  const { rows } = await sql`
    ...
    RETURNING *
  `;
  await logActivity(req, { action: 'update', entityType: 'member', entityId: rows[0].id, description: `Updated member "${rows[0].name}"` });
  res.json(await serializeMember(rows[0]));
});

app.delete('/api/members/:id', async (req, res) => {
  const { rows: existingRows } = await sql`SELECT name FROM members WHERE id = ${req.params.id}`;
  const { rowCount } = await sql`DELETE FROM members WHERE id = ${req.params.id}`;
  if (!rowCount) return res.status(404).json({ error: 'member not found' });
  await logActivity(req, { action: 'delete', entityType: 'member', entityId: req.params.id, description: `Removed member "${existingRows[0]?.name}"` });
  res.status(204).end();
});
...
app.delete('/api/members/:id/growth/:growthId', async (req, res) => {
  ...
  await logActivity(req, { action: 'delete', entityType: 'growth_entry', entityId: req.params.growthId, description: 'Removed a growth entry' });
  res.status(204).end();
});
...
app.put('/api/queue/:slot', async (req, res) => {
  ...
  `;
  await logActivity(req, { action: 'update', entityType: 'queue_slot', entityId: slot, description: `Updated the ${slot} insignia queue` });
...
app.post('/api/boss-timers', async (req, res) => {
  ...
  const { rows } = await sql`
    ...
    RETURNING *
  `;
  await logActivity(req, { action: 'create', entityType: 'boss_timer', entityId: rows[0].id, description: `Added boss timer "${rows[0].name}"` });
...
app.put('/api/boss-timers/:id', async (req, res) => {
  ...
  const { rows } = await sql`
    ...
    RETURNING *
  `;
  await logActivity(req, { action: 'update', entityType: 'boss_timer', entityId: rows[0].id, description: `Updated boss timer "${rows[0].name}"` });
...
app.delete('/api/boss-timers/:id', async (req, res) => {
  const { rows: existingRows } = await sql`SELECT name FROM boss_timers WHERE id = ${req.params.id}`;
  ...
  await logActivity(req, { action: 'delete', entityType: 'boss_timer', entityId: req.params.id, description: `Removed boss timer "${existingRows[0]?.name}"` });
  res.status(204).end();
});
...
app.post('/api/item-categories', async (req, res) => {
  ...
  await logActivity(req, { action: 'create', entityType: 'item_category', entityId: id, description: `Added item "${trimmed}"` });
...
app.put('/api/item-categories/:id', async (req, res) => {
  ...
  await logActivity(req, { action: 'update', entityType: 'item_category', entityId: req.params.id, description: `Updated item "${nextName}"` });
  res.json({ id: req.params.id, name: nextName, iconUrl: nextIcon });
});

app.delete('/api/item-categories/:id', async (req, res) => {
  const { rows: existingRows } = await sql`SELECT name FROM item_categories WHERE id = ${req.params.id}`;
  const { rowCount } = await sql`DELETE FROM item_categories WHERE id = ${req.params.id}`;
  if (!rowCount) return res.status(404).json({ error: 'item not found' });
  await logActivity(req, { action: 'delete', entityType: 'item_category', entityId: req.params.id, description: `Removed item "${existingRows[0]?.name}"` });
  res.status(204).end();
});
...
app.delete('/api/loot/:id', async (req, res) => {
  ...
  await logActivity(req, { action: 'delete', entityType: 'loot_session', entityId: req.params.id, description: 'Deleted a Guild Dungeon date' });
  res.status(204).end();
});
...
app.post('/api/loot/:id/raffle-log', async (req, res) => {
  ...
  await logActivity(req, { action: 'create', entityType: 'raffle_log', entityId: req.params.id, description: entry.message });
...
app.delete('/api/loot/:id/raffle-log', async (req, res) => {
  ...
  await logActivity(req, { action: 'delete', entityType: 'raffle_log', entityId: req.params.id, description: 'Cleared the raffle activity log' });
  res.status(204).end();
});
...
app.delete('/api/loot/:id/records/:recordId', async (req, res) => {
  const { rows: existingRows } = await sql`SELECT item FROM loot_records WHERE id = ${req.params.recordId} AND session_id = ${req.params.id}`;
  const { rowCount } = await sql`
  ...
  await logActivity(req, {
    ...
    description: `Removed loot "${existingRows[0]?.item}"`,
  });
  res.status(204).end();
});
...
app.delete('/api/caves/:id', async (req, res) => {
  ...
  await logActivity(req, { action: 'delete', entityType: 'cave_session', entityId: req.params.id, description: 'Deleted a cave boss run' });
  res.status(204).end();
});
...
app.post('/api/caves/:id/records', async (req, res) => {
  ...
  `;
  const row = rows[0];
  await logActivity(req, { action: 'create', entityType: 'cave_record', entityId: row.id, description: `Added cave loot "${row.item}" (x${row.quantity})` });
  res.status(201).json({
    id: row.id,
    item: row.item,
    quantity: Number(row.quantity),
...
app.put('/api/caves/:id/records/:recordId', async (req, res) => {
  ...
  `;
  const row = rows[0];
  await logActivity(req, { action: 'update', entityType: 'cave_record', entityId: row.id, description: `Updated cave loot "${row.item}" (x${row.quantity})` });
  res.json({
    id: row.id,
    item: row.item,
    quantity: Number(row.quantity),
...
app.delete('/api/caves/:id/records/:recordId', async (req, res) => {
  const { rows: existingRows } = await sql`SELECT item FROM cave_records WHERE id = ${req.params.recordId} AND session_id = ${req.params.id}`;
  const { rowCount } = await sql`
    DELETE FROM cave_records WHERE id = ${req.params.recordId} AND session_id = ${req.params.id}
  `;
  if (!rowCount) return res.status(404).json({ error: 'record not found' });
  await logActivity(req, { action: 'delete', entityType: 'cave_record', entityId: req.params.recordId, description: `Removed cave loot "${existingRows[0]?.item}"` });
  res.status(204).end();
});
...
app.delete('/api/salary-paid', async (req, res) => {
  ...
  await logActivity(req, { action: 'update', entityType: 'salary_paid', entityId: memberId, description: `Unmarked a member as paid for ${month}` });
  res.status(204).end();
});
