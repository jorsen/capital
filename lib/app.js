  ...
  try {
    ...
    for (const msg of messages) {
      ...
      if (explicit && explicitBoss) {
        killedAtByBossId.set(explicitBoss.id, explicit.killedAt);
        await sql`
...
app.delete('/api/boss-history/:id', async (req, res) => {
  ...
  // If this was the most recent kill logged for its boss, boss_timers.last_killed_at
  // was set from it — deleting it without also rolling the timer back would leave
  // the countdown pointed at a kill that no longer appears in the log at all.
  let revertedBossId = null;
  let revertedTo = null;
  if (entry.boss_id) {
    const { rows: currentTopRows } = await sql`
      SELECT id FROM boss_kill_history WHERE boss_id = ${entry.boss_id} ORDER BY killed_at DESC LIMIT 1
    `;
    if (currentTopRows[0]?.id === entry.id) {
