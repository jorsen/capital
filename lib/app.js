...
// site's audit trail. The clear itself still gets logged under the same
// entityType, so the log isn't left with zero explanation for why it's empty.
app.delete('/api/activity-log', async (req, res) => {
  ...
  const { rows: countRows } = await sql`SELECT COUNT(*)::int AS count FROM activity_log WHERE entity_type = ${entityType}`;
  await sql`DELETE FROM activity_log WHERE entity_type = ${entityType}`;
  await logActivity(req, {
    action: 'delete',
    entityType,
    entityId: 'all',
    description: `🧹 Cleared the ${entityType.replace(/_/g, ' ')} activity log (${countRows[0].count} entr${countRows[0].count === 1 ? 'y' : 'ies'})`,
  });
  res.status(204).end();
});
...
// Winning on Defense pays 40% of a team's reward out to whoever bid gold on
// the PREVIOUS team -- a thank-you to last time's bidders, funded by this
// time's defenders. Every team is its own independent battle now, so
// "previous" walks across every crusade/team globally (ordered by the
// parent crusade's event date, then its created_at, then team number),
// not just teams within the same crusade. Returns both which team that was
// (for display) and its bidders.
async function getLastTeamBidders(crusade, team) {
  ...
  const { rows: prevRows } = await sql`
    SELECT ct.team_number, c.id AS crusade_id, c.name AS crusade_name, c.event_date
    FROM crusade_teams ct
    JOIN crusades c ON c.id = ct.crusade_id
    WHERE c.event_date < ${crusade.event_date}
       OR (c.event_date = ${crusade.event_date} AND c.created_at < ${crusade.created_at})
       OR (c.event_date = ${crusade.event_date} AND c.created_at = ${crusade.created_at} AND ct.team_number < ${team.team_number})
    ORDER BY c.event_date DESC, c.created_at DESC, ct.team_number DESC
    LIMIT 1
  `;
  const prev = prevRows[0];
  if (!prev) return { lastTeam: null, bidders: [] };
  const { rows: bidderRows } = await sql`
    SELECT name, guild_name, gold_bid FROM crusade_participants
    WHERE crusade_id = ${prev.crusade_id} AND party_number = ${prev.team_number} AND gold_bid > 0
    ORDER BY name ASC
  `;
  return {
    lastTeam: { crusadeId: prev.crusade_id, crusadeName: prev.crusade_name, teamNumber: prev.team_number, eventDate: prev.event_date },
