...
async function fetchCaveRecords(sessionIds) {
  if (sessionIds.length === 0) return new Map();
  const { rows } = await sql`
    SELECT id, session_id, item, quantity, notes, sent, created_at
    FROM cave_records
    WHERE session_id = ANY(${sessionIds}::uuid[])
    ORDER BY created_at ASC
  `;
...
app.post('/api/caves/:id/records', async (req, res) => {
  ...
  const trimmedItem = item.trim();
  const { rows } = await sql`
    INSERT INTO cave_records (id, session_id, item, quantity, notes)
    VALUES (${id}, ${req.params.id}, ${trimmedItem}, ${qty}, ${trimmedNotes})
    RETURNING *
  `;
  const row = rows[0];
  res.status(201).json({
...
app.put('/api/caves/:id/records/:recordId', async (req, res) => {
  ...
  const { item, quantity, notes, sent } = req.body || {};
  ...
  const { rows } = await sql`
    UPDATE cave_records
    SET item = ${nextItem}, quantity = ${nextQuantity}, notes = ${nextNotes}, sent = ${nextSent}
    WHERE id = ${req.params.recordId}
    RETURNING *
  `;
  const row = rows[0];
