...
function serializeCrusadeItem(row) {
  return { id: row.id, crusadeId: row.crusade_id, name: row.name, quantity: Number(row.quantity) };
}

function serializeCrusadeFee(row) {
  return { id: row.id, crusadeId: row.crusade_id, name: row.name, guildName: row.guild_name, percent: Number(row.percent) };
}
...
// Winning on Defense pays 40% of the reward out to whoever bid gold in the
// PREVIOUS crusade (by event date) -- a thank-you to last time's bidders,
// funded by this time's defenders. "Previous" means the closest earlier
// event_date across every crusade, not just crusades this guild fought.
// Returns both which crusade that was (for display) and its bidders.
async function getLastCrusadeBidders(crusade) {
  if (!crusade.event_date) return { lastCrusade: null, bidders: [] };
  const { rows: prevRows } = await sql`
    SELECT id, name, event_date FROM crusades
    WHERE event_date < ${crusade.event_date} AND id != ${crusade.id}
    ORDER BY event_date DESC
    LIMIT 1
  `;
  const prev = prevRows[0];
  if (!prev) return { lastCrusade: null, bidders: [] };
  const { rows: bidderRows } = await sql`
    SELECT name, guild_name, gold_bid FROM crusade_participants
    WHERE crusade_id = ${prev.id} AND gold_bid > 0
    ORDER BY name ASC
  `;
  return {
    lastCrusade: { id: prev.id, name: prev.name, eventDate: prev.event_date },
    bidders: bidderRows.map((r) => ({ name: r.name, guildName: r.guild_name, goldBid: Number(r.gold_bid) })),
  };
}
...
app.get('/api/crusades', async (req, res) => {
  const { rows } = await sql`
    SELECT c.*,
      ...
      (SELECT COALESCE(SUM(percent), 0) FROM crusade_fees f WHERE f.crusade_id = c.id) AS total_fee_percent
    FROM crusades c
    ORDER BY c.created_at DESC
  `;
  res.json(
    rows.map((r) => {
      // Mirrors the client-side math (management fees off the top, nothing
      // at all if the crusade was lost) so the totals shown here match what
      // the Teams list and Guild Salary page actually pay out — not the raw
      // diamond_reward value, which is only the gross figure before fees.
      const netDiamondReward =
        r.result === 'lose' ? 0 : Number(r.diamond_reward) * Math.max(0, 1 - Number(r.total_fee_percent) / 100);
      return { ...serializeCrusade(r), participantCount: r.participant_count, netDiamondReward };
    })
  );
});

app.post('/api/crusades', async (req, res) => {
  const { name, warType, stance, area, leader, result, diamondReward, attendancePct, eventDate, notes } = req.body || {};
  if (!name || !name.trim()) return res.status(400).json({ error: 'name is required' });

  const finalDiamondReward = Number(diamondReward) || 0;
  if (finalDiamondReward < 0) return res.status(400).json({ error: 'diamondReward must be a non-negative number' });
  const finalAttendancePct = attendancePct !== undefined && attendancePct !== null && attendancePct !== '' ? Number(attendancePct) : 50;
  if (Number.isNaN(finalAttendancePct) || finalAttendancePct < 0 || finalAttendancePct > 100) {
    return res.status(400).json({ error: 'attendancePct must be between 0 and 100' });
  }

  const id = crypto.randomUUID();
  await sql`
    INSERT INTO crusades (id, name, war_type, stance, area, leader, result, diamond_reward, attendance_pct, event_date, notes)
    VALUES (
      ${id}, ${name.trim()}, ${warType || null}, ${stance || null}, ${area || null}, ${leader || null},
      ${result || 'pending'}, ${finalDiamondReward}, ${finalAttendancePct}, ${eventDate || null}, ${notes || null}
    )
  `;
  const { rows } = await sql`SELECT * FROM crusades WHERE id = ${id}`;
  ...
  res.status(201).json({ ...serializeCrusade(rows[0]), participantCount: 0, netDiamondReward: finalDiamondReward });
});

app.get('/api/crusades/:id', async (req, res) => {
  const { rows } = await sql`SELECT * FROM crusades WHERE id = ${req.params.id}`;
  ...
  `;
  const { rows: itemRows } = await sql`SELECT * FROM crusade_items WHERE crusade_id = ${req.params.id} ORDER BY created_at ASC`;
  const { rows: feeRows } = await sql`SELECT * FROM crusade_fees WHERE crusade_id = ${req.params.id} ORDER BY created_at ASC`;
  const { lastCrusade, bidders: lastCrusadeBidders } = await getLastCrusadeBidders(crusade);
  res.json({
    ...serializeCrusade(crusade),
    participants: participantRows.map(serializeCrusadeParticipant),
    items: itemRows.map(serializeCrusadeItem),
    fees: feeRows.map(serializeCrusadeFee),
    lastCrusade,
    lastCrusadeBidders,
  });
});
...
app.put('/api/crusades/:id', async (req, res) => {
  const { rows: existingRows } = await sql`SELECT * FROM crusades WHERE id = ${req.params.id}`;
  const existing = existingRows[0];
  ...
  const { rows } = await sql`
    UPDATE crusades SET
      name = ${nextName}, war_type = ${nextWarType}, stance = ${nextStance}, area = ${nextArea},
      leader = ${nextLeader}, result = ${nextResult}, diamond_reward = ${nextDiamondReward},
      attendance_pct = ${nextAttendancePct}, event_date = ${nextEventDate}, notes = ${nextNotes}
    WHERE id = ${req.params.id}
    RETURNING *
  `;
  await logActivity(req, {
...
app.post('/api/crusades/:id/items', async (req, res) => {
  ...
  const id = crypto.randomUUID();
  await sql`INSERT INTO crusade_items (id, crusade_id, name, quantity) VALUES (${id}, ${req.params.id}, ${name.trim()}, ${finalQuantity})`;
  const { rows } = await sql`SELECT * FROM crusade_items WHERE id = ${id}`;
  await logActivity(req, {
    action: 'create',
    entityType: 'crusade_item',
    entityId: id,
    description: `Added item "${name.trim()}" (${finalQuantity}) to crusade`,
    after: { name: name.trim(), quantity: finalQuantity },
  });
  res.status(201).json(serializeCrusadeItem(rows[0]));
});

app.put('/api/crusades/:id/items/:itemId', async (req, res) => {
  const { rows: existingRows } = await sql`SELECT * FROM crusade_items WHERE id = ${req.params.itemId} AND crusade_id = ${req.params.id}`;
  const existing = existingRows[0];
  if (!existing) return res.status(404).json({ error: 'item not found' });
...
app.delete('/api/crusades/:id/items/:itemId', async (req, res) => {
  const { rows: existingRows } = await sql`SELECT name FROM crusade_items WHERE id = ${req.params.itemId} AND crusade_id = ${req.params.id}`;
  const { rowCount } = await sql`DELETE FROM crusade_items WHERE id = ${req.params.itemId} AND crusade_id = ${req.params.id}`;
  if (!rowCount) return res.status(404).json({ error: 'item not found' });
  await logActivity(req, {
    action: 'delete',
    entityType: 'crusade_item',
    entityId: req.params.itemId,
    description: `Removed item "${existingRows[0]?.name}" from crusade`,
    before: existingRows[0] ? { name: existingRows[0].name } : undefined,
  });
  res.status(204).end();
});

app.post('/api/crusades/:id/fees', async (req, res) => {
  const { rows: crusadeRows } = await sql`SELECT id FROM crusades WHERE id = ${req.params.id}`;
  if (!crusadeRows.length) return res.status(404).json({ error: 'crusade not found' });
  ...
  const id = crypto.randomUUID();
  await sql`
    INSERT INTO crusade_fees (id, crusade_id, name, guild_name, percent)
    VALUES (${id}, ${req.params.id}, ${name.trim()}, ${guildName || null}, ${finalPercent})
  `;
  ...
  await logActivity(req, {
    ...
    entityId: id,
    description: `Added a ${finalPercent}% management fee for "${name.trim()}"`,
    after: { name: name.trim(), guildName: guildName || null, percent: finalPercent },
  });
  res.status(201).json(serializeCrusadeFee(rows[0]));
});

app.delete('/api/crusades/:id/fees/:feeId', async (req, res) => {
  const { rows: existingRows } = await sql`SELECT name, percent FROM crusade_fees WHERE id = ${req.params.feeId} AND crusade_id = ${req.params.id}`;
  const { rowCount } = await sql`DELETE FROM crusade_fees WHERE id = ${req.params.feeId} AND crusade_id = ${req.params.id}`;
  if (!rowCount) return res.status(404).json({ error: 'fee not found' });
  await logActivity(req, {
    action: 'delete',
    entityType: 'crusade_fee',
    entityId: req.params.feeId,
    description: `Removed the management fee for "${existingRows[0]?.name}"`,
...
function serializeSovereignMember(row) {
  return { id: row.id, name: row.name, guildName: row.guild_name, updatedAt: row.updated_at };
}
...
// whichever guild was most recently entered for them. Never throws: a
// failure here shouldn't take down the participant save that triggered it.
async function upsertSovereignMember(name, guildName) {
  const trimmed = (name || '').trim();
  if (!trimmed) return;
  try {
    const { rows: existing } = await sql`SELECT id FROM sovereign_members WHERE LOWER(name) = LOWER(${trimmed})`;
    if (existing.length) {
      await sql`UPDATE sovereign_members SET name = ${trimmed}, guild_name = ${guildName || null}, updated_at = now() WHERE id = ${existing[0].id}`;
    } else {
      await sql`INSERT INTO sovereign_members (id, name, guild_name) VALUES (${crypto.randomUUID()}, ${trimmed}, ${guildName || null})`;
    }
  } catch (err) {
...
app.post('/api/crusades/:id/participants', async (req, res) => {
  ...
  await upsertSovereignMember(rows[0].name, rows[0].guild_name);
  await logActivity(req, {
    action: 'create',
