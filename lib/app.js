...
// defenders. "Last time" always means the most recent OTHER crusade (a
// separate calendar event, by event date) -- never a sibling team within
// the SAME crusade, since teams on the same crusade are simultaneous
// waves of one event, not a separate "last time." Every Attack team of
// that other crusade that actually WON counts, not just its single
// highest-numbered one -- a losing team's bidders get nothing, since they
// never actually took the reward they'd have bid on. A Defense-stance
// source team is excluded too, since it never had bidding to begin with.
// Returns both which crusade that was (for display) and the pooled
// bidder list.
async function getLastTeamBidders(crusade) {
  ...
  const sourceCrusadeId = prevCrusade.id;
  const { rows: sourceTeams } = await sql`
    SELECT team_number, stance, result FROM crusade_teams WHERE crusade_id = ${sourceCrusadeId}
  `;

  const lastTeam = { crusadeId: sourceCrusadeId, crusadeName: prevCrusade.name, eventDate: prevCrusade.event_date };
  const attackTeamNumbers = sourceTeams.filter((t) => t.stance !== 'Defense' && t.result === 'win').map((t) => t.team_number);
...
app.get('/api/crusades/:id', async (req, res) => {
  ...
  // Every team on this crusade shares the same "last crusade" bonus source
  // (it's crusade-level, not team-level), so this only needs to run once.
  const { lastTeam, bidders: lastTeamBidders } = await getLastTeamBidders(crusade);
  const teams = [];
...
async function processCaveAttendanceMessages(messages, { channelId, botToken, year }) {
  ...
  const [{ rows: bosses }, { rows: members }, { rows: loggedRows }] = await Promise.all([
    ...
    sql`SELECT message_id, edited_at FROM cave_attendance_logged_messages WHERE message_id = ANY(${messages.map((m) => m.id)})`,
  ]);
  const bossNames = bosses.map((b) => b.name).sort((a, b) => b.length - a.length);
  const loggedEditedAtByMessageId = new Map(loggedRows.map((r) => [r.message_id, r.edited_at]));

  const results = [];
  for (const msg of messages) {
    if (loggedEditedAtByMessageId.has(msg.id)) {
      const loggedEditedAt = loggedEditedAtByMessageId.get(msg.id);
      const editedSinceLogged = msg.edited_timestamp && (!loggedEditedAt || new Date(msg.edited_timestamp) > new Date(loggedEditedAt));
    ...
    // Some bosses spawn more than once in a single day, so matching on
    // date + boss name alone would merge two genuinely different kills into
    // one session -- a fixed "close enough in time" window doesn't work
    // here either, since respawn intervals vary per boss and can easily be
    // shorter than any window generous enough to also cover a same-kill
    // duplicate/correction posted well after the fact. Instead: only merge
    // into an existing same-day/same-boss session when this message's
    // stated kill time matches that session's EXACTLY (down to the minute)
    // -- two people confirming the same kill naturally state the same time,
    // while a genuinely separate spawn (even minutes apart) states a
    // different one and gets its own session. A message with no parsed
    // kill time at all falls back to whichever existing session also has
    // no recorded time, since there's no better signal to go on there.
    const killedAt = explicit ? explicit.killedAt : null;
    const { rows: candidateSessions } = await sql`
      SELECT * FROM cave_sessions WHERE date = ${date} AND LOWER(run) = LOWER(${bossName}) ORDER BY created_at ASC
    `;
    let session = killedAt
      ? candidateSessions.find((s) => s.killed_at && new Date(s.killed_at).getTime() === new Date(killedAt).getTime())
      : null;
    if (!session) session = candidateSessions.find((s) => !s.killed_at) || null;

    const mergedAttendees = Array.from(new Set([...(session?.attendees || []), ...matchedIds]));

    if (session) {
      const { rows } = await sql`
        UPDATE cave_sessions SET attendees = ${JSON.stringify(mergedAttendees)}, killed_at = COALESCE(killed_at, ${killedAt}) WHERE id = ${session.id} RETURNING *
      `;
      session = rows[0];
    } else {
      const { rows } = await sql`
        INSERT INTO cave_sessions (id, date, run, attendees, killed_at)
        VALUES (${crypto.randomUUID()}, ${date}, ${bossName}, ${JSON.stringify(mergedAttendees)}, ${killedAt})
        RETURNING *
      `;
      session = rows[0];
    }
    ...
    // the message's current edited_timestamp (if any) so a later edit is
    // detected as needing another pass instead of being skipped forever.
    await sql`
      INSERT INTO cave_attendance_logged_messages (message_id, edited_at) VALUES (${msg.id}, ${msg.edited_timestamp || null})
      ON CONFLICT (message_id) DO UPDATE SET edited_at = ${msg.edited_timestamp || null}
    `;
