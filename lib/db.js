...
function ensureSchema() {
  if (!schemaReady) {
    schemaReady = (async () => {
      ...
      // Lets an item stay in the catalog (kept for historical loot records,
      // reports, and icon lookups) while being left out of the item-picker
      // dropdown used to log new loot -- distinct from deleting it outright,
      // which would orphan any past record still referencing that name.
      await sql`ALTER TABLE item_categories ADD COLUMN IF NOT EXISTS hidden BOOLEAN NOT NULL DEFAULT false`;
      await sql`
      ...
      // World Dungeon Salary — mirrors cave_sessions/cave_records exactly
      // (own boss run log + sold loot), so the same "sum of qty × sold_price
      // for the month" pool math applies. Kept as its own tables rather than
      // reusing cave_sessions since World Dungeon runs are a distinct
      // activity (Thursday/Sunday world bosses, not cave bosses) with their
      // own attendance and loot pool.
      await sql`
        CREATE TABLE IF NOT EXISTS world_dungeon_sessions (
          id UUID PRIMARY KEY,
          date TEXT NOT NULL,
          run TEXT NOT NULL DEFAULT '',
          notes TEXT NOT NULL DEFAULT '',
      ...
      `;
      // A run can also drop diamonds directly (not just sellable loot) --
      // tracked as its own pool alongside the crow (🐦‍⬛) loot-sold pool,
      // split the same way (attendance + multiplier) but never mixed
      // together, since they're different currencies. Added after the fact,
      // hence ALTER rather than a column in the CREATE TABLE above.
      await sql`ALTER TABLE world_dungeon_sessions ADD COLUMN IF NOT EXISTS diamond_reward NUMERIC NOT NULL DEFAULT 0`;
      await sql`
        CREATE TABLE IF NOT EXISTS world_dungeon_records (
          id UUID PRIMARY KEY,
          session_id UUID NOT NULL REFERENCES world_dungeon_sessions(id) ON DELETE CASCADE,
          item TEXT NOT NULL,
          quantity NUMERIC NOT NULL DEFAULT 1,
      ...
      // Single standing conversion rate (not month-scoped) for showing a
      // real-money PHP column next to the crow (🐦‍⬛) salary figures --
      // e.g. 33 means 1 crow = ₱33. Diamonds aren't converted here since
      // only crows were asked to be.
      await sql`
        CREATE TABLE IF NOT EXISTS world_dungeon_settings (
          id INTEGER PRIMARY KEY DEFAULT 1,
          php_per_crow NUMERIC NOT NULL DEFAULT 0,
          updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
        )
      `;

      // Whether a member has GCash on file -- a standing per-member flag
      // (not month-scoped), same idea as the Multiplier table below; anyone
      // without a row defaults to "no GCash" in the app layer.
      await sql`
        CREATE TABLE IF NOT EXISTS world_dungeon_gcash (
          member_id UUID PRIMARY KEY REFERENCES members(id) ON DELETE CASCADE,
          has_gcash BOOLEAN NOT NULL DEFAULT false,
          updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
        )
      `;

      // A flat per-member multiplier, entered directly by an admin (unlike
      // Cave Salary's growth-rate tier lookup) — one row per member who has
      // ever had a non-default value set; anyone without a row falls back to
      // 1 in the app layer. Not month-scoped: intended as a standing weight
      // per member (e.g. rank/role), same as the flat "Multiplier" column in
      // the guild's original spreadsheet.
      await sql`
        CREATE TABLE IF NOT EXISTS world_dungeon_multipliers (
          member_id UUID PRIMARY KEY REFERENCES members(id) ON DELETE CASCADE,
      ...
      // PVP Attendance Bonus -- a standing (not month-scoped, like
      // world_dungeon_multipliers above) rolling list of recent PVP/crusade
      // dates the admin tracks attendance for. Each member's flat Multiplier
      // above gets scaled by (dates they attended ÷ dates currently listed)
      // before it's applied to the World Dungeon salary split, so missing
      // PVPs quietly shrinks their World Dungeon cut too. Deleting a date
      // (once its period is over) cascades its attendance rows with it.
      await sql`
        CREATE TABLE IF NOT EXISTS world_dungeon_pvp_dates (
          id UUID PRIMARY KEY,
          date DATE NOT NULL,
          created_at TIMESTAMPTZ NOT NULL DEFAULT now()
        )
      `;
      await sql`
        CREATE TABLE IF NOT EXISTS world_dungeon_pvp_attendance (
          id UUID PRIMARY KEY,
          pvp_date_id UUID NOT NULL REFERENCES world_dungeon_pvp_dates(id) ON DELETE CASCADE,
          member_id UUID NOT NULL REFERENCES members(id) ON DELETE CASCADE,
          attended BOOLEAN NOT NULL DEFAULT true,
          UNIQUE (pvp_date_id, member_id)
        )
      `;

      // Same accounting-fee / paid-tracking shape as cave_salary_fees /
      // cave_salary_paid, kept as separate tables since the two salary pools
      // (Cave vs. World Dungeon) are accounted independently per month.
      await sql`
        CREATE TABLE IF NOT EXISTS world_dungeon_salary_fees (
          id UUID PRIMARY KEY,
          month TEXT NOT NULL,
          name TEXT NOT NULL,
          member_id UUID REFERENCES members(id) ON DELETE SET NULL,
      ...
      `;
      await sql`
        CREATE TABLE IF NOT EXISTS world_dungeon_salary_paid (
          id UUID PRIMARY KEY,
          month TEXT NOT NULL,
          member_id UUID NOT NULL REFERENCES members(id) ON DELETE CASCADE,
          created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
      ...
      `;
      // Tracks whether a top-20 (by Growth Rate) member has been sent one of
      // the handful of items Item Report restricts itself to (Morion, Frozen
      // Tear, Orb of Winds) -- same shape as world_dungeon_salary_paid, just
      // keyed by item name instead of month.
      await sql`
        CREATE TABLE IF NOT EXISTS item_send_status (
          id UUID PRIMARY KEY,
          item_name TEXT NOT NULL,
          member_id UUID NOT NULL REFERENCES members(id) ON DELETE CASCADE,
          created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
          UNIQUE (item_name, member_id)
        )
      `;
      ...
      `;
      // Which Discord message a 'discord'-sourced row came from, so a
      // message can never get logged (or Discord-confirmed) twice even if
      // the poll cursor below fails to save and re-fetches the same batch,
      // or two poll runs happen to overlap.
      await sql`ALTER TABLE boss_kill_history ADD COLUMN IF NOT EXISTS message_id TEXT`;
