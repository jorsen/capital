...
function ensureSchema() {
  if (!schemaReady) {
    schemaReady = (async () => {
      ...
      `;
      // Named items per crusade (e.g. Morion x215, Guild Coins x500), each
      // split evenly across attendees only -- same "attended is a must" rule
      // as the diamond attendance portion, just for physical item counts.
      await sql`
      ...
      // A management fee takes a percentage of the crusade's total diamond
      ...
      // fees (see cave_salary_fees), just per-crusade and keyed by IGN
