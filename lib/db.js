...
function ensureSchema() {
  if (!schemaReady) {
    schemaReady = (async () => {
      ...
      // across attendees only -- same idea as the diamond attendance
      // portion, just for a physical item count instead of diamonds.
      await sql`ALTER TABLE crusades ADD COLUMN IF NOT EXISTS item_name TEXT`;
      await sql`ALTER TABLE crusades ADD COLUMN IF NOT EXISTS item_quantity NUMERIC NOT NULL DEFAULT 0`;
      await sql`
