    ...
    schemaReady = (async () => {
      ...
      await sql`
        CREATE TABLE IF NOT EXISTS loot_sessions (
          ...
          raffle_winner_id UUID REFERENCES members(id) ON DELETE SET NULL,
          created_at TIMESTAMPTZ NOT NULL DEFAULT now()
        )
      `;
      ...
      await sql`ALTER TABLE loot_sessions ADD COLUMN IF NOT EXISTS raffle_winner_id UUID REFERENCES members(id) ON DELETE SET NULL`;
      await sql`
