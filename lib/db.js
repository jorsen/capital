...
function ensureSchema() {
  if (!schemaReady) {
    schemaReady = (async () => {
      ...
      // existing item/fee onto every one of those teams. Guarded by
      // crusade_teams being completely empty, so this only ever runs once —
      // after it runs, no crusade_id-only (team_id IS NULL) item/fee rows
      // are left for it to find, so it's safe to leave this block in place.
      const { rows: teamMigrationCheck } = await sql`SELECT COUNT(*)::int AS count FROM crusade_teams`;
      if (teamMigrationCheck[0].count === 0) {
